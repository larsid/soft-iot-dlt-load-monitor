...
public class Processor {
    ...
    protected void updateBrokerStatus(Integer qtdDevices) throws InterruptedException {
        ...
        System.out.println("Amount of devices: "+ qtdDevices + " | Need balancing? "+ lbEntry);

        if(lbEntry) { //Se o gateway está sobrecarregado.
          if(flag || System.currentTimeMillis() > this.lastSentLbEntry + (lbEntryTimeout * 1000)) {
            // Salva o tempo da última vez que enviou uma transação do tipo LB_ENTRY.
            this.lastSentLbEntry = System.currentTimeMillis();
            this.flag = false;
            
            this.connector.put(qtdDevices, true, qtdDevices.longValue(), qtdDevices.longValue(),available);
          }
        } else {
          this.connector.put(qtdDevices, false, qtdDevices.longValue(), qtdDevices.longValue(),available);
          this.flag = true;
        }
    }
